...
class CVExperimentPlanner(object):
    def __init__(self, dataset_name_or_id: Union[str, int],
                 gpu_memory_target_in_gb: float = 8,
                 preprocessor_name: str = 'DefaultPreprocessor', plans_name: str = 'nnUNetPlans',
                 overwrite_target_spacing: Union[List[float], Tuple[float, ...]] = None,
        ...
        preprocessed_folder = join(nnUNet_preprocessed, self.dataset_name)
        self.dataset_json = load_json(join(self.raw_dataset_folder, 'dataset.json'))
        self.dataset = get_filenames_of_train_images_and_targets_CV(self.raw_dataset_folder, self.dataset_json)
        self.fold_idx = fold_idx

        # load dataset fingerprint
        if not isfile(join(preprocessed_folder, f'dataset_fingerprint_fold{fold_idx}.json')):
            raise RuntimeError('Fingerprint missing for this dataset. Please run nnUNet_extract_dataset_fingerprint')

        self.dataset_fingerprint = load_json(join(preprocessed_folder, f'dataset_fingerprint_fold{fold_idx}.json'))
    ...
    def determine_reader_writer(self):
        example_image = self.dataset[str(self.fold_idx)]["training"][0]['image']
        return determine_reader_writer_from_dataset_json(self.dataset_json, example_image)
    ...
    def save_plans(self, plans):
        ...
        plans_file = join(nnUNet_preprocessed, self.dataset_name, self.plans_identifier + f"fold{self.fold_idx}" + '.json')
        ...
        maybe_mkdir_p(join(nnUNet_preprocessed, self.dataset_name))
        save_json(plans, plans_file, sort_keys=False)
        print(f"Plans were saved to {join(nnUNet_preprocessed, self.dataset_name, self.plans_identifier + f"fold{self.fold_idx}" + '.json')}")
